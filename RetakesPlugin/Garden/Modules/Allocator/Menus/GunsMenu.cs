...
public class GunsMenu : AbstractBaseMenu
{
    ...
    private string ZeusAlwaysOption => Translator.Instance["menu.zeus.always"];
    private string ZeusNeverOption => Translator.Instance["menu.zeus.never"];

    private void OpenZeusMenu(CCSPlayerController player)
    {
        var menu = new ChatMenu($"{MessagePrefix}{Translator.Instance["guns_menu.zeus_menu"]}");

        menu.AddMenuOption(ZeusAlwaysOption, OnZeusSelect);
        menu.AddMenuOption(ZeusNeverOption, OnZeusSelect);
    ...
    private void OnZeusSelect(CCSPlayerController player, ChatMenuOption option)
    {
        if (!PlayersInMenu.Contains(player))
        {
            return;
        ...
        var enabled = option.Text == ZeusAlwaysOption;
        Queries.SetZeusPreference(Helpers.GetSteamId(player), enabled);
        AllocatorModule.Instance?.ApplyZeusChangeNow(player, enabled);

        Print(player, Translator.Instance[enabled ? "zeus.enabled" : "zeus.disabled"]);
    ...
    public static void HandlePreferenceSelection(CCSPlayerController player, CsTeam team, string weapon,
        ...
        bool remove = false)
    {
        var roundType = roundTypeOverride ??
                        RoundTypeManager.Instance.GetCurrentEffectiveRoundType(player.Team);
        ...
        // If the selection is valid for the ongoing round and the buy window is still open,
        // swap the weapon immediately.
        if (!remove && selectedWeapon is not null && player.Team == team)
        {
            AllocatorModule.Instance?.ApplyImmediateWeaponChange(player, selectedWeapon.Value);
        }
    }
}
