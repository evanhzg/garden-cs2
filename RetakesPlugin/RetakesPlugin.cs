...
{
    public const string Version = "3.0.4-garden.1";

    #region Plugin Info
    public override string ModuleName => "Garden Retakes";
    public override string ModuleVersion => Version;
    public override string ModuleAuthor => "Evan (Garden) — based on B3none's cs2-retakes";
    public override string ModuleDescription => "Garden Retakes — signature plugin (see ROADMAP.md)";
    #endregion
    ...
    private GardenHost? _garden;

    public GardenHost? Garden => _garden;

    public MapConfigService? MapConfigService => _mapConfigService;
    public SpawnManager? SpawnManager => _spawnManager;

    /// <summary>Garden (R1): lets the spawn editor module force a bombsite for dry-run rounds.</summary>
    public RoundEventHandlers? RoundHandlers => _roundEventHandlers;

    /// <summary>
    /// Garden (R4): the retakes round/queue machinery only runs while the
    /// Retakes mode is active — Duels/Executes/FastStrat own the round flow.
    /// </summary>
    public bool RetakesGameplayActive =>
        _garden is null || _garden.Modes.CurrentMode == GameModeKind.Retakes;

    /// <summary>
    /// Garden (R12): while a Competitive Retakes match (or its team-pick setup)
    /// runs, the retakes win-rotation/scramble/balancing must NOT touch teams —
    /// CR owns the sides. Fixes the inverted-sides bug (CTs spawning as Ts).
    /// </summary>
    public bool SuspendTeamManagement { get; set; }
    #endregion
    ...
    /// <summary>
    /// The internal retakes event sender. Garden modules (eg. the allocator)
    /// subscribe to this directly instead of going through the capability.
    /// </summary>
    ...
    public override void Load(bool hotReload)
    {
        ...
        Capabilities.RegisterPluginCapability(RetakesPluginEventSenderCapability, () => EventSender);

        // Garden module system (ROADMAP R0) — commands + event handlers register once here.
        _garden = new GardenHost(this);
        _garden.Load(hotReload);
