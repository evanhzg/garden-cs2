...
public class MapConfigService
{
    ...
    /// <summary>
    /// Garden (ROADMAP R1): mutate the live spawn list in place (move, flag
    /// toggle, ...), then persist and reload. Single in-memory authority, so
    /// several admins editing at once stay consistent.
    /// </summary>
    public void MutateSpawns(Action<List<Spawn>> mutate)
    {
        _mapConfigData ??= new MapConfigData();
        mutate(_mapConfigData.Spawns);
        Save();
        Load();
    }
