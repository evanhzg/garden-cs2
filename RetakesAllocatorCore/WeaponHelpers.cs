...
public static class WeaponHelpers
{
    ...
    // Item definition indexes (CEconItemView.ItemDefinitionIndex) -> CsItem.
    // Used to identify purchases without relying on the fragile
    // GetCSWeaponDataFromKey native (whose signature breaks on CS2 updates).
    private static readonly Dictionary<int, CsItem> _itemsByDefinitionIndex = new()
    {
        {1, CsItem.Deagle}, {2, CsItem.Dualies}, {3, CsItem.FiveSeven}, {4, CsItem.Glock},
        {7, CsItem.AK47}, {8, CsItem.AUG}, {9, CsItem.AWP}, {10, CsItem.Famas},
        {11, CsItem.AutoSniperT}, {13, CsItem.Galil}, {14, CsItem.M249}, {16, CsItem.M4A4},
        {17, CsItem.Mac10}, {19, CsItem.P90}, {23, CsItem.MP5}, {24, CsItem.UMP45},
        {25, CsItem.XM1014}, {26, CsItem.Bizon}, {27, CsItem.MAG7}, {28, CsItem.Negev},
        {29, CsItem.SawedOff}, {30, CsItem.Tec9}, {31, CsItem.Taser}, {32, CsItem.P2000},
        {33, CsItem.MP7}, {34, CsItem.MP9}, {35, CsItem.Nova}, {36, CsItem.P250},
        {38, CsItem.AutoSniperCT}, {39, CsItem.Krieg}, {40, CsItem.Scout},
        {42, CsItem.KnifeCT}, {43, CsItem.Flashbang}, {44, CsItem.HEGrenade}, {45, CsItem.Smoke},
        {46, CsItem.Molotov}, {47, CsItem.Decoy}, {48, CsItem.Incendiary}, {49, CsItem.C4},
        {59, CsItem.KnifeT}, {60, CsItem.M4A1S}, {61, CsItem.USPS}, {63, CsItem.CZ}, {64, CsItem.R8},
    };

    public static CsItem? GetItemFromDefinitionIndex(int definitionIndex)
    {
        return _itemsByDefinitionIndex.TryGetValue(definitionIndex, out var item) ? item : null;
    }
    ...
    public static bool IsWeaponAllocationAllowed(bool isFreezePeriod)
    {
        ...
        if (!Configs.GetConfigData().AllowAllocationAfterFreezeTime)
        {
            return false;
        }

        // After freeze time, weapon changes are only applied immediately during a short buy window.
        return RoundTypeManager.Instance.IsInWeaponChangeWindow(
            Configs.GetConfigData().WeaponChangeWindowAfterRoundStartSeconds
        );
    }

    /// <summary>
    /// Whether a pistol counts as the team's "default" for the pistol-round economy rule.
    /// USP-S and P2000 are interchangeable CT defaults.
    /// </summary>
    public static bool IsDefaultPistol(CsTeam team, CsItem weapon)
    {
        var defaultPistol = GetDefaultPistol(team);
        if (defaultPistol is null)
        {
            return false;
        }

        if (weapon == defaultPistol)
        {
            return true;
        }

        return defaultPistol is CsItem.USPS or CsItem.P2000 && weapon is CsItem.USPS or CsItem.P2000;
    }

    /// <summary>
    /// The configured default pistol-round pistol for a team (used by the pistol-round economy rule).
    /// </summary>
    public static CsItem? GetDefaultPistol(CsTeam team)
    {
        ...
        if (
            Configs.GetConfigData().DefaultWeapons.TryGetValue(team, out var teamDefaults) &&
            teamDefaults.TryGetValue(WeaponAllocationType.PistolRound, out var configuredDefault)
        )
        {
            return configuredDefault;
        }

        return _defaultWeaponsByTeamAndAllocationType[team][WeaponAllocationType.PistolRound];
    }
}
